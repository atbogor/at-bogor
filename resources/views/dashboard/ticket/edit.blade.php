...
<div class="container mt-4">
    ...
    <form method="post" action="/dashboard/tickets/{{ $ticket->slug }}" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="mb-3">
            <label for="title" class="form-label">Title</label>
            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title"
                required autofocus value="{{ old('title', $ticket->title) }}">
            @error('title')
        ...
        <div class="mb-3">
            ...
            <select name="category_id"
                ...
                value="{{ old('category_id', $ticket->category_id) }}">
                <option disabled {{ old('category_id', $ticket->category_id) === null ? 'selected' : '' }}>Category
                </option>
                @foreach ($ticketcategories as $ticketcategory)
                    @if(old('category_id', '$ticket->category_id') == $ticketcategory->id)
        ...
        <div class="mb-3">
            ...
            <input type="hidden" name="oldImage" value="{{ $ticket->image }}">
            @if($ticket->image)
                <img SRC="{{ asset('storage/' . $ticket->image) }}" class="img-preview img-fluid mb-3 col-sm-5 d-block">
            @else
        ...
        <button class="w-100 btn btn-secondary btn-primary mt-3 mb-3" type="submit">Edit Ticket</button>
    </form>
</div>
